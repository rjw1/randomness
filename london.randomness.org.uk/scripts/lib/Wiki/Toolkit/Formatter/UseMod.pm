package Wiki::Toolkit::Formatter::UseMod;
...
A formatter backend for L<Wiki::Toolkit> that supports UseMod-style formatting.
...
A L<Wiki::Toolkit> object can be supplied as an optional second parameter.
...
delimiters.  C<wiki> is optional but should be a L<Wiki::Toolkit> object.
    ...
    my %format_tags = ( extended_link_delimiters => [ '[[', ']]' ],
                        link => sub {
                            ...
                            $link = $self->{_munge_node_name}($link)
                              if $self->{_munge_node_name};
                            $link = $self->_munge_spaces($link);
                            push @Wiki::Toolkit::Formatter::UseMod::_links_found,
                                                                         $link;
